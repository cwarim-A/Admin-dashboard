...
export const ProductForm: React.FC<ProductFormProps> = ({ 
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
      ...
      if(initialData){
        console.log(data)
         await axios.patch(`${baseURL}/api/${params.storeId}/products/${params.productId}`, data);
      }else{
        await axios.post(`${baseURL}/api/${params.storeId}/products`, data);
      }
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`${baseURL}/api/${params.storeId}/products/${params.productId}`);
      router.refresh();
      router.push(`/${params.storeId}/products`);
        ...
        >
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    ...
                    disabled={loading}
                    // onChange={(url)=>field.onChange((prev:{url:string}[]=[])=>[...(prev || []),{url}] )}
          ...
          <div className="grid grid-cols-3 gap-8">
            ...
            <FormField
              ...
              name="isFeatured"
              render={({ field }) => (
                <FormItem className="flex flex-rows items-start space-x-3 space-y-0 rounded-md border p-4">
                    <FormControl>
                      <Checkbox
                      checked={field.value}
                      // @ts-ignore
